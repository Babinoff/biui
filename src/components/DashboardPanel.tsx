...
export function DashboardPanel() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Temporarily set presentation mode to true if it's not
  ...
  const renderContent = () => {
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2 text-purple-400">
              ...
              <h3 className="font-semibold text-slate-200">Dashboard Presentation</h3>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={handleExportPDF}
                className="flex items-center gap-1 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-semibold rounded transition-colors"
                title="Export to PDF"
              >
                <Download size={14} />
                PDF
          ...
          <div id="dashboard-container" className="flex-1 bg-slate-950 rounded-lg border border-slate-700 overflow-hidden relative">
            <DashboardEditor />
          </div>
        </div>
      );
    ...
    if (selectedNode?.type === 'visualization') {
      const libraryId = selectedNode.data?.outputLibraryId || selectedNode.data?.libraryId || 'echarts';
      const chartConfig = selectedNode.data?.outputChartConfig || null;
        ...
        <div className="flex flex-col h-full gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-emerald-400">
              <BarChart3 size={20} />
              <h3 className="font-semibold text-slate-200">Visualization Full View</h3>
            </div>
          ...
          <div className="flex-1 border border-slate-700 rounded-lg bg-slate-950 p-4 relative">
            {chartConfig ? (
          ...
          <div className="bg-slate-800 rounded-lg p-3 border border-slate-700">
            <h4 className="text-xs font-semibold text-slate-300 mb-2">Generated Configuration</h4>
            <pre className="text-[10px] text-slate-400 font-mono overflow-auto max-h-40 custom-scrollbar">
              {selectedNode.data?.generatedConfig || '// No configuration generated yet'}
            </pre>
          </div>
        </div>
      );
      ...
      return (
        <div className="flex flex-col h-full gap-4">
          <div className="flex items-center gap-2 text-orange-400">
            <Eye size={20} />
            <h3 className="font-semibold text-slate-200">Watch Data</h3>
          </div>
          
          {headers.length > 0 ? (
            <div className="flex-1 overflow-auto border border-slate-700 rounded-md bg-slate-800/50 custom-scrollbar">
              <table className="w-full text-left border-collapse text-xs">
                <thead className="sticky top-0 bg-slate-800 shadow-sm z-10">
                  <tr>
                    {headers.map((header: string, i: number) => (
                      <th key={i} className="p-2 border-b border-r border-slate-700 font-medium text-slate-300 whitespace-nowrap">
                        {header}
                      </th>
                    ))}
                  </tr>
                  ...
                  {data.map((row: any[], rowIndex: number) => (
                    <tr key={rowIndex} className="hover:bg-slate-800/50 transition-colors">
                      {row.map((cell: any, cellIndex: number) => (
                        <td key={cellIndex} className="p-2 border-b border-r border-slate-700/50 text-slate-400 whitespace-nowrap max-w-[200px] truncate" title={String(cell)}>
                          {String(cell)}
                        </td>
                      ))}
                    </tr>
            ...
            <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-slate-700 rounded-lg text-slate-500">
    ...
    if (selectedNode?.type === 'transform') {
      return (
        <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-slate-700 rounded-lg text-slate-500 mb-4">
          <Cpu size={24} className="mb-2 opacity-50 text-blue-400" />
          <span className="text-sm">Transform Properties</span>
          <span className="text-xs text-slate-600 mt-2 text-center px-4">Use the node interface on the canvas to edit prompts and code.</span>
        </div>
      );
    }
    ...
    if (selectedNode?.type === 'dataSource') {
      return (
        <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-slate-700 rounded-lg text-slate-500 mb-4">
          <Database size={24} className="mb-2 opacity-50 text-indigo-400" />
    ...
    if (selectedNode?.type === 'dashboard') {
      return (
        <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-slate-700 rounded-lg text-slate-500 mb-4 text-center px-4">
          <LayoutDashboard size={24} className="mb-2 opacity-50 text-purple-400" />
          <span className="text-sm">Dashboard Node</span>
          <span className="text-xs text-slate-600 mt-2">Click "Add to Dashboard" on the node to add visualizations. Deselect to view the dashboard.</span>
        </div>
      );
    }
  ...
  return (
    <div
      className={`relative h-full bg-slate-900 text-slate-200 border-l border-slate-800 transition-all duration-300 ease-in-out flex flex-col shrink-0 ${
    ...
    >
      ...
      <button
        ...
        className="absolute -left-3 top-6 bg-slate-800 border border-slate-700 text-slate-400 hover:text-white rounded-full p-1 z-10 shadow-md transition-colors cursor-pointer"
      >
        {isOpen ? <ChevronRight size={14} /> : <ChevronLeft size={14} />}
      </button>

      <div className={`flex items-center justify-between p-4 border-b border-slate-800 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0 overflow-hidden whitespace-nowrap'}`}>
        <div className="flex items-center gap-2 font-semibold">
          {selectedNode ? <Settings2 size={18} className="text-blue-400" /> : <LayoutDashboard size={18} className="text-emerald-400" />}
          <span>{selectedNode ? 'Properties' : 'Dashboard'}</span>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={handleMaximize} className="text-slate-400 hover:text-white p-1 rounded-md hover:bg-slate-800 transition-colors cursor-pointer">
