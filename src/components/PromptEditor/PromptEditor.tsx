...
export function PromptEditor({ nodeId }: { nodeId: string }) {
  ...
  const history: PromptHistoryItem[] = node?.data?.history || [];
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (actualSourceNode?.type === 'dataSource' && actualSourceNode.data.selectedSourceId) {
        ...
        inputHeaders = actualSourceNode.data.outputHeaders;
        inputData = actualSourceNode.data.outputData;
      }
      ...
      const result = await PythonRunner.run(currentCode, inputHeaders, inputData, (msg) => addLog(msg, 'info'));
  ...
  return (
    <div className="flex flex-col gap-3 w-full nodrag cursor-default">
      ...
      <div className="flex flex-col gap-1">
        <div className="relative">
          <textarea
            ...
            className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-blue-500 resize-none h-16 custom-scrollbar nodrag"
            onKeyDown={(e) => {
          ...
          <button
            ...
            className="absolute bottom-2 right-2 p-1 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded transition-colors"
        ...
        {error && (
          <div className="text-[10px] text-red-400 bg-red-950/50 border border-red-900/50 p-1.5 rounded">
            {error}
          </div>
        )}
      </div>
      ...
      {/* Logs Panel */}
      {showLogs && (
        <div className="flex flex-col gap-1 border border-slate-700 rounded overflow-hidden bg-black h-24 shrink-0">
          <div className="flex items-center justify-between bg-slate-800/80 px-2 py-1">
      ...
      <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            {!showLogs && logs.length > 0 && (
              <button 
                ...
                className="text-[10px] text-slate-400 hover:text-slate-300 transition-colors flex items-center gap-1"
              >
                <Terminal size={10} />
                Show Logs
              </button>
            ...
            <button 
              ...
              className="flex items-center gap-1 text-[10px] bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 px-1.5 py-0.5 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div className="border border-slate-700 rounded overflow-hidden bg-[#1e1e1e] relative h-32">
          {currentCode ? (
            <SyntaxHighlighter
              language="python"
              style={vscDarkPlus}
              customStyle={{ margin: 0, padding: '0.5rem', height: '100%', fontSize: '0.75rem' }}
      ...
      {/* History Panel */}
      {history.length > 0 && (
        <div className="border border-slate-700 rounded overflow-hidden shrink-0">
          <button 
            onClick={() => setShowHistory(!showHistory)}
            className="w-full flex items-center gap-1.5 p-1.5 bg-slate-800/80 hover:bg-slate-700 text-[10px] text-slate-300 transition-colors"
          >
          ...
          {showHistory && (
            <div className="max-h-48 overflow-y-auto custom-scrollbar bg-slate-900 p-1.5 flex flex-col gap-1.5">
              {history.slice().reverse().map((item) => (
                <div 
                  key={item.id} 
                  className="p-1.5 border border-slate-700 rounded bg-slate-800/50 hover:bg-slate-800 transition-colors group flex flex-col gap-1"
                >
                  <div 
                    className="flex justify-between items-start cursor-pointer"
                    onClick={() => setExpandedHistoryId(expandedHistoryId === item.id ? null : item.id)}
                  >
                    <div className="text-[10px] text-slate-300 line-clamp-2 group-hover:text-blue-400 transition-colors flex-1 pr-2">
                      {item.prompt}
                    </div>
                  ...
                  {expandedHistoryId === item.id && (
                    <div className="mt-1 pt-1 border-t border-slate-700/50">
                      <div className="text-[9px] text-slate-400 mb-1">Full Response:</div>
                      <div className="bg-slate-950 p-1.5 rounded border border-slate-800 text-[10px] text-slate-300 max-h-24 overflow-y-auto custom-scrollbar nodrag whitespace-pre-wrap">
                        {item.rawResponse}
                      </div>
                      <button 
                        onClick={(e) => { e.stopPropagation(); handleRestoreHistory(item); }}
                        className="mt-1.5 w-full py-1 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 text-[10px] rounded transition-colors"
                      >
