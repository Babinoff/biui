...
export function VisualizationNode({ id }: { id: string }) {
  ...
  const libraryId = node?.data?.libraryId || 'echarts';
  const chartType = node?.data?.chartType || 'bar';
  const prompt = node?.data?.prompt || '';
  const generatedConfig = node?.data?.generatedConfig || '';
  const promptHistory = node?.data?.promptHistory || [];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const configData = await LLMClient.generateChartConfig(
        ...
        inputHeaders,
        inputData,
        prompt || `Create a ${chartType} chart`,
        (msg) => addLog(msg, 'info')
      );
      ...
      updateNodeData(id, { 
        ...
        promptHistory: [newHistoryItem, ...promptHistory],
      });
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-lg w-80 flex flex-col">
      <Handle type="target" position={Position.Left} className="w-3 h-3 bg-emerald-500 border-2 border-slate-800" />
      
      <div className="bg-emerald-900/50 p-2 border-b border-slate-700 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <BarChart2 size={14} className="text-emerald-400" />
          <span className="text-xs font-semibold text-slate-200">Visualization</span>
        </div>
        <button onClick={() => setShowConfig(!showConfig)} className="text-slate-400 hover:text-slate-200">
          <Settings2 size={14} />
        </button>
      </div>

      {showConfig && (
        <div className="p-3 border-b border-slate-700 flex flex-col gap-3 nodrag cursor-default shrink-0">
          <div className="flex gap-2">
            <div className="flex-1 flex flex-col gap-1">
              <label className="text-[10px] text-slate-400 uppercase">Library</label>
              <select 
                value={libraryId}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] text-slate-400 uppercase">Prompt (Optional)</label>
            <textarea
              value={prompt}
              onChange={(e) => updateNodeData(id, { prompt: e.target.value })}
              placeholder="e.g., Show revenue by month..."
              className="w-full bg-slate-900 border border-slate-700 rounded p-1.5 text-xs text-slate-200 placeholder:text-slate-600 resize-none h-12 custom-scrollbar"
            />
          </div>

          {/* Logs Panel */}
          {showLogs && (
            <div className="flex flex-col gap-1 bg-black rounded border border-slate-700 h-24 shrink-0 overflow-hidden">
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 text-white text-xs rounded transition-colors"
            >
      ...
      {/* Code Editor Area */}
      <div className="flex flex-col border-b border-slate-700 bg-slate-950 h-48 shrink-0 nodrag">
        <div className="flex items-center justify-between bg-slate-900 px-2 py-1 border-b border-slate-800">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-0.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 text-white text-[10px] rounded transition-colors"
          >
            <Play size={10} />
            Run
          </button>
        ...
        <textarea
          ...
          spellCheck={false}
          className="flex-1 w-full bg-transparent text-slate-300 text-[10px] font-mono p-2 resize-none focus:outline-none custom-scrollbar"
      ...
      {promptHistory.length > 0 && (
        <div className="flex flex-col border-b border-slate-700 bg-slate-900 h-24 shrink-0 nodrag">
          <div className="flex items-center justify-between bg-slate-800/80 px-2 py-1">
            <div className="flex items-center gap-1 text-[10px] text-slate-400 font-semibold">
              <History size={10} />
              Prompt History
          ...
          <div className="p-1.5 overflow-y-auto custom-scrollbar flex flex-col gap-1">
            {promptHistory.map((item: any) => (
              <div 
                key={item.id} 
                className="bg-slate-800 p-1.5 rounded border border-slate-700 hover:border-emerald-500 cursor-pointer transition-colors"
                onClick={() => {
                  updateNodeData(id, { 
                    prompt: item.prompt,
              ...
              >
                <div className="flex justify-between items-center mb-0.5">
                  ...
                  <span className="text-[8px] px-1 py-0.5 bg-slate-700 text-slate-300 rounded">
